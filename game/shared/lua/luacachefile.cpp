...
LUA_API void luasrc_ExtractLcf()
{
	...
	for ( int i = 0; i < downloadables->GetNumStrings(); i++ )
	{
		...
		if ( !Q_stricmp( ext, "lcf" ) )
		{
			...
			{
				Warning( "Failed to open LCF zip: %s\n", fullpath );
				return;
			}

			int numitems = (int)mz_zip_reader_get_num_files( &zip );
			for ( int i = 0; i < numitems; i++ )
			{
				mz_zip_archive_file_stat ze;
				if ( !mz_zip_reader_file_stat( &zip, i, &ze ) )
					continue;
				...
				char path[MAX_PATH];
				Q_snprintf( path, sizeof( path ), LUA_PATH_CACHE "/%s", ze.m_filename );
				Q_StripFilename( path );
				filesystem->CreateDirHierarchy( path, "MOD" );

				char outFullpath[MAX_PATH];
				Q_snprintf( outFullpath, sizeof( outFullpath ), "%s%s", cachePath, path + Q_strlen( LUA_PATH_CACHE ) );

				V_SetCurrentDirectory( outFullpath );

				if ( !mz_zip_reader_extract_to_file( &zip, i, V_UnqualifiedFileName( ze.m_filename ), 0 ) )
				{
...
static const luaL_Reg lcf_funcs[] = { { "sendfile", luasrc_sendfile }, { NULL, NULL } };
